...
def feasibleroute(graph,vertex,visited,maxlength,targetvertex):
    visited[vertex] = 1
    for neighbour in graph[vertex]:
        node = neighbour[0]
        weight = neighbour[1]
        if(visited[node]!=1 and weight>=maxlength):
            if(node==targetvertex):
                return "there is the root"
            return feasibleroute(graph,node,visited,maxlength,targetvertex)
...
def main():
    ...
    print(feasibleroute(G,1,visited,3,4))
