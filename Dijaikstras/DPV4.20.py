...
def calculate_distances(graph,starting_vertex,pq,t,length):
    vertex_list = []
    ...
    weight = 0
    flag = False
    while tracker!=starting_vertex:
        tpred = vertex_list[tracker].pred
        nodes = graph[tracker]
        # print(tpred)
        for node in nodes:
            if(node[0]==tpred):
                ...
                if(weight>length):
                    flag =True
                    print("edge can be added at ",(tracker,tpred)," to reduce the cost by",vertex_list[tracker].dist-weight+length)
        tracker = tpred

    if not flag:
        print("the path can not be reduced with existing length change the length")
...
def main():
    ...
    calculate_distances(G,s,pq,t,1)
